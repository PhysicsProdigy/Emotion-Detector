...
def convolutional_block(img, f, filters, stage, block, s=2):
    ...
    img_copy = Conv2D(F2, (1, 1), strides=(s, s), padding='valid', name=conv_name_base + '1',
...
def ResNet34(input_shape=(64, 64, 1), classes=7):
    ...
    # Stage 2
    img = convolutional_block(img, f=3, filters=[64, 64], stage=2, block='a', s=1)
    img = identity_block(img, 3, [64, 64], stage=2, block='b')
    img = identity_block(img, 3, [64, 64], stage=2, block='c')

    # Stage 3
    img = convolutional_block(img, f=3, filters=[128, 128], stage=3, block='a', s=2)
    img = identity_block(img, 3, [128, 128], stage=3, block='b')
    img = identity_block(img, 3, [128, 128], stage=3, block='c')
    img = identity_block(img, 3, [128, 128], stage=3, block='d')

    # Stage 4
    img = convolutional_block(img, f=3, filters=[256, 256], stage=4, block='a', s=2)
    img = identity_block(img, 3, [256, 256], stage=4, block='b')
    img = identity_block(img, 3, [256, 256], stage=4, block='c')
    img = identity_block(img, 3, [256, 256], stage=4, block='d')
    img = identity_block(img, 3, [256, 256], stage=4, block='e')
    img = identity_block(img, 3, [256, 256], stage=4, block='f')

    # Stage 5
    img = convolutional_block(img, f=3, filters=[512, 512], stage=5, block='a', s=2)
    img = identity_block(img, 3, [512, 512], stage=5, block='b')
    img = identity_block(img, 3, [512, 512], stage=5, block='c')
...
model = ResNet34(input_shape=(64,64,1), classes = 7)
...
checkpoint = ModelCheckpoint("resNet34_Weights.h5", monitor='val_accuracy',
...
with open("resNet34.json", "w") as json_file:
